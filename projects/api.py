...
##################################
        if (str(self.request.user)=="admin@example.com"):
            projects = Project.objects.all()
        else:
            from assignment1.models import Assignment
            filter_p = Assignment.objects.filter(assigned_to=str(self.request.user))
            fl = []
            for i in filter_p:
                print(i.project_id)
                fl.append(i.project_id)

            projects = Project.objects.filter(id__in=fl)
##################################
    ...
    def perform_create(self, ser):
      if str(self.request.user) == 'admin@example.com':
        try:
          ...
          raise PermissionError('You are not authorized to create project')
    ...
    def perform_destroy(self, instance):
      ...
      if str(self.request.user) == 'admin@example.com':
        with temporary_disconnect_all_signals():
          ...
          raise PermissionError('You are not authorized to delete project')
